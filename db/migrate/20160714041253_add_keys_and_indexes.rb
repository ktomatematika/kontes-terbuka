...
class AddKeysAndIndexes < ActiveRecord::Migration
  def change
    %i[start_time end_time result_time feedback_time].each do |t|
    ...
    add_index :long_problems, %i[contest_id problem_no], unique: true
    ...
    add_index :long_submissions, %i[long_problem_id user_contest_id],
              unique: true
    ...
    add_index :short_problems, %i[contest_id problem_no], unique: true

    add_index :short_submissions, %i[short_problem_id user_contest_id],
              unique: true
    ...
    add_index :submission_pages, %i[page_number long_submission_id],
              unique: true

    add_foreign_key :temporary_markings, :users
    add_foreign_key :temporary_markings, :long_submissions
    add_index :temporary_markings, %i[user_id long_submission_id],
              unique: true
    ...
    add_index :user_contests, %i[user_id contest_id], unique: true
