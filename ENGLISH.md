...
8. Create `.env` file in the root directory and copy over the contents from `.env.default` to `.env`
9. The initial setup to run the server has been done. To run the server in test environment, you can run 
    ```bash
